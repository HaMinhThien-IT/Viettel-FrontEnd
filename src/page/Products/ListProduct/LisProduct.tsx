...
export default function LisProduct() {
    ...
    useEffect(() => {
        productController.listProduct(1, 3, '', '', 1, 100000000).then(res => {
    ...
    const handleChange = (event: React.ChangeEvent<unknown>, value: number) => {
        setPage(value)
        productController.listProduct(value, 3, '', '', 1, 100000000).then(res => {
            setState({ ...state, product: res.listProduct, countPage: res.totalPage })
    ...
    const setOrder = (orderby: string) => {
        ...
        if (orderby === '') {
            console.log(orderby);
            productController.listProduct(1, 3, '', '', 1, 100000000).then(res => { setState({ ...state, product: res.listProduct, countPage: res.totalPage }) })
        } else {
            productController.listProduct(1, 3, '', orderby, 1, 100000000).then(res => {
                setState({ ...state, product: res.listProduct, countPage: res.totalPage })
            })
        }
    }
